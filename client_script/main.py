...
if args.input and args.output:
    with open(str(args.input), newline = '', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        with open(str(args.output), 'a', newline='') as file:
            fieldnames = ['Name', 'transactionHash', 'status', 'to', 'blockNumber']
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
            for row in reader:
                print(row['Name'])
                process = subprocess.run(
                    ['node', 'script.js', row['Name'], row['University']],
                    text=True,
                    stdout=subprocess.PIPE,
                    check=True
                )
                for line in process.stdout.split():
                    nameConverted = { "Name": row['Name'] }
